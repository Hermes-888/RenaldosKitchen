...
func _on_GameUI_ready():
	#print("GUI ready first")
	instructions = $GameUI/GridRow/CenterUI/instructions
	tempSlider = $GameUI/GridRow/CenterUI/RightUI/tempSlider
	againBtn = $GameUI/GridRow/CenterUI/againBtn
...
func resetVariables():
	...
	doorOpened = false
	tempSlider.value = 80
	$GameUI/GridRow/CenterUI/RightUI/temp.text = "80"
	$GameUI/GridRow/CenterUI/againBtn.visible = false
	$GameUI/GridRow/CenterUI/RightUI.visible = false
	$Pointsprite.visible = false
	$GameUI/thumbHand.visible = false
...
func sliderChanged(value):
	#var e = get_node("GameUI/GridRow/CenterUI/RightUI/tempSlider").value
	$GameUI/GridRow/CenterUI/RightUI/temp.text = str(value)
	#get_node("GameUI/GridRow/CenterUI/RightUI/temp").text = str(value)
	var r = 0.0
	var b = 0.3
	...
	if !sliderUsed and value > 40 and value < 70:
		...
		# show thumbs up
		$GameUI/thumbHand.visible = true
		# add points
		$Pointsprite.visible = true
		$AnimationPlayer.play("showPointsprite")
		$World/AudioStreamPlayer.play()
		points += 10
		$GameUI/GridRow/CenterUI/LeftUI/score.text = "SCORE: " + str(points)

func changeInstructions(index):
	stepIndex = index
	$GameUI/GridRow/CenterUI/instructions.text = steps[index] # array of strings
...
func _on_Handle_Hot_mouse_entered():
	...
	if !waterHot:
		...
		$GameUI/GridRow/CenterUI/RightUI.visible = true
		# Too HOT color red. set slider value too high, points for slider lowered
		tempSlider.value = 170
		$GameUI/GridRow/CenterUI/RightUI/temp.text = "170"
...
func _on_SpoutArea_mouse_entered():
	...
	actionCount = actionCount + 1
	if actionCount > 20:
		completed = true # trigger Step 4: Rinse
	...
	var stween = $World/Tween
	var ftween = $World/Tween
	...
	stween.interpolate_property($SpoutBase, "rotation_degrees", 
	...
	ftween.interpolate_property($hand_sink/Faucet_spout, "rotation_degrees", 
		Vector3(0,faucetFrom,0), Vector3(0,rotateTo,0), dur, 
...
func _on_Door_Handle_mouse_entered():
	...
	if stepIndex == 6 and doorOpened:
		$hand_sink/Door_wooden/Door_handle.transform.basis = Basis(Vector3(0, 0, 1), 0)
		return
	...
	doorOpened = true
	$hand_sink/Door_wooden/Door_handle.transform.basis = Basis(Vector3(0, 0, 1), -13)
#	var ftween = $World/Tween
...
func _on_Trash_mouse_entered():
	if stepIndex == 7:
		...
		$GameUI/GridRow/CenterUI/againBtn.visible = true
		# set cursor to cursors[stepIndex]
